...
fun PsiElement.resolvePhpClasses(useIndex: Boolean = false): List<PhpClass> {
    ...
    // Complete the type to resolve short names (e.g. Map -> \Noctud\Collection\Map\Map)
    val resolvedType = index.completeType(project, typed.type, null)
    log("resolvePhpClasses: raw types=${typed.type.types}, resolved types=${resolvedType.types}")

    val result = LinkedHashSet<PhpClass>()
    for (type in resolvedType.types.filter { it.startsWith("\\") }) {
        result.addAll(index.getClassesByFQN(type))
        result.addAll(index.getInterfacesByFQN(type))
    }
